...
const CarouselContent = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & { carouselClass?: string }
>(({ carouselClass, className, ...props }, ref) => {
  const { carouselRef, orientation } = useCarousel();

  return (
    <div ref={carouselRef} className={cn("overflow-hidden", carouselClass)}>
      <div
        ref={ref}
        className={cn(
          "flex",
...
>((props, ref) => {
  ...
  if (numberOfSlides > 1) {
    return (
      <div ref={ref} className={`flex justify-end ${props.className ?? ""}`}>
        {Array.from({ length: numberOfSlides }, (_, i) => (
          <Button
            ...
            className={`mx-1 h-1.5 w-1.5 rounded-full p-0 ${
              i === currentSlide
                ? "scale-125 transform bg-blue-500 hover:bg-blue-500"
